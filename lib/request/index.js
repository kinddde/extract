...
class Request {
    ...
    startRequest(prequestPram) {
        let { url, method = "GET", response = "json", param, encode = "qs", paramType = "json", paramRule, urlTemplate = false } = prequestPram;
        ...
        if (urlTemplate) {
            this.$url = lodash_1.template(this.$url)(this.$param);
        }
        if (this.$method === "GET") {
            return this.get();
        }
